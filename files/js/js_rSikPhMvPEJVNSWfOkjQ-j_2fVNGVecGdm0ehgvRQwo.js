 ...
 */
Drupal.behaviors.vertical_tabs_FieldsetSummaries = {
  ...
  var $mlids = [];
  var $i = 0;
  $links = $("a[id*='-operations-delete']");
  $links.click(function(e) {
    $mlids_str = $('input#mlids-to-delete').val();
    $mlids = $mlids_str.split(',');
    $mlid = e.target.id.match(/[0-9]+/g);
    if ($(e.target).hasClass('undo')) {
      $i = $mlids.indexOf($mlid[0]);
      if ($i > -1) {
        $mlids.splice($i, 1);
      }
      $(e.target).text('delete');
      $(e.target).removeClass('undo');
      $(e.target).closest('tr').removeClass('hybrid-disabled');
    }
    else {
      $mlids.push($mlid[0]);
      $(e.target).text('undo');
      $(e.target).addClass('undo');
      $(e.target).closest('tr').addClass('hybrid-disabled');
    }
    $('input#mlids-to-delete').val($mlids.join(','));
    e.preventDefault();
  });

  $container = $('input#mlid-to-delete').val();

  /**
   * Compare trees.
   */
  var $container = $('fieldset#edit-revision');
  if ($container.length > 0) {
    var $button = $("#edit-revision-compare-modal a");
    var $checkboxes = $('fieldset#edit-revision input[type="checkbox"]');
    var new_args = args = [];
    $container.click(function(e) {
      var selected = [];
      for (i = 0; i < $checkboxes.length; i++) {
        if ($checkboxes[i].checked) {
          selected.push($($checkboxes[i]).attr('value'));
        }
      }
      if (2 == selected.length) {
        
        args = $button.attr('href').split("/");
        console.log();
        new_args = [args[1], "ajax", args[3], selected[1], selected[0]];
        Drupal.ajax[$button.attr('href')].options.url = "/" + new_args.join("/");
//        console.log(Drupal.ajax[$button.attr('href')].options);
//        $button.attr('href', "/" + new_args.join("/"));
      }
    });
  }
  }
