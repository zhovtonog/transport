...
(function($) {
	$(function() {
		var info = [],
			settings = Drupal.settings.blocks || Drupal.settings.steps || Drupal.settings.nodes || [];
		
		if ( settings ) {
			for( var sid in settings ) {
				info.push({id:sid, text:null == settings[sid] ? '': settings[sid]})
			}
		}
		
		$('input.su-flow-add-select2').select2(
			{
				multiple: true,
				data: info,
				width: '100%',
		...
		$("input.su-flow-add-select2").each(function() {
			$(this).select2("container").find("ul.select2-choices").sortable({
				containment: 'parent',
				start: function() { $(this).select2("onSortStart"); },
				update: function(key){
					return function() { $(key).select2("onSortEnd"); }
				}(this)
			});
		});

        var step = $('.error.messages:contains("Steps is empty")').length;
        if ( 0 !== step ){
            $('.su-flow-add-select2').css('border', '2px solid red');
        }
	});
})(jQuery);
(function($){
	...
	var issetBtn = function(){
		//var steps = $('input[name="content"]').val();
		//return ( -1 != steps.indexOf(981) );
		var regions = ['sidebar-right','content','sidebar-left'];
		for (var region in regions) {
			if ( $('input[name="'+regions[region]+'"]').val().indexOf(981) != -1 ) {
				return true;
			}
		}
	...
	var showModal = function(){
		$('#continue-btn-alter').foundation('reveal', 'open');
	};

	var hideModal = function(){
		$('#continue-btn-alter').foundation('reveal', 'close');
	};

	var setActionModal = function(){
		$('#ok').click(function(){
			status = true;
			hideModal();
			//$('form').submit();
			$('#sushi-flow-add-step-btn').click();
		});
	...
	$(function(){
		setActionModal();
		$('#sushi-flow-add-step-btn').click(function(e){
      if(!issetBtn() && status === false){
        showModal();
        //no submit
        e.preventDefault();
        e.stopPropagation();
